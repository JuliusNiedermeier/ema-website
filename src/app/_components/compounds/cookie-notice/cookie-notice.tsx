...
export const CookieNotice: FC<CookieNoticeProps> = ({ className, ...restProps }) => {
  ...
  const progress = useTransform(scrollY, [0, 500, 800], [100, 100, 0]);
  const transformY = useTransform(progress, (value) => `${value}%`);
  const opacity = useTransform(progress, [100, 0], [0, 1]);

  return (
    <Card
      className={cn("w-fit max-w-96 border border-neutral-400/20 bg-primary-900", className)}
      {...restProps}
      asChild
    >
      <motion.div style={{ y: transformY, opacity }}>
